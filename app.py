...
sub_title = """
            <div>
                ...
                text-align:center;
                margin-top:-40px;">
                An Automatic Machine Learning Tool for Computer Vision Projects
 </h6>
            </div>
            """
...
def main():
    ...
    if input_source == "Image":
        ...
        image = st.sidebar.file_uploader("Select input image", type=["jpg", "png", "jpeg"], accept_multiple_files=False)
        ...
        if st.sidebar.button("Start Detection"):
            if image is not None:  # Check if image is uploaded
                ...
                st.markdown("""<h4 style="color:black;">
                      Memory Overall Statistics</h4>""", unsafe_allow_html=True)
                kpi5, kpi6 = st.columns(2)

                with kpi5:
                    st.markdown("""<h5 style="color:black;">
                          CPU Utilization</h5>""", unsafe_allow_html=True)
                    kpi5_text = st.markdown("0")

                with kpi6:
                    st.markdown("""<h5 style="color:black;">
                          Memory Usage</h5>""", unsafe_allow_html=True)
                    kpi6_text = st.markdown("0")

                # Read the image using OpenCV
                img = cv2.imdecode(np.fromfile(image.name, dtype=np.uint8), cv2.IMREAD_COLOR)

                detect(weights=weights,
                       source=image.name,  # Use frame instead of source for image
                       stframe=stframe,
                       kpi5_text=kpi5_text,
                       kpi6_text=kpi6_text,
